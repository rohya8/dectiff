...
public class ScheduledUser extends Fragment implements AndroidConstants {
			...
			public void onClick(DialogInterface dialog, int which) {
				if (!Validation.isNetworkAvailable(getActivity())) {
					...
					if (i == 1) {
						...
						new ScheduleCancelOrderTask(getActivity(), customerOrder, switchorder, cancelorder, tiffstatus, viewmenu, tiffmenu).execute();
					} else if (i == 2) {
						Toast.makeText(getActivity(), "2 !!", Toast.LENGTH_SHORT).show();
						new ScheduleCancelOrderTask(getActivity(), customerOrder, card2switchorder, card2cancelorder, tiffcard2status, card2viewmenu,
	...
	private void initialise() {
		...
		card2cancelorder = (Button) view.findViewById(R.id.scheduled_user_homescreen_card2_cancel_button);
		card2switchorder = (Button) view.findViewById(R.id.scheduled_user_homescreen_card2_switch_button);
	...
	/*private void showCard1(CustomerOrder customerOrder1) {
		...
		if (customerOrder1.getMealStatus() == null || MealStatus.PREPARE.equals(customerOrder1.getStatus()))
			tiffstatus.setText("Your " + customerOrder1.getMealType() + " is yet to be cooked. You can cancel your order.");
		else
			tiffstatus.setText("Your " + customerOrder1.getMealType() + " is in " + customerOrder1.getMealStatus().toString() + " stage.");
		...
		tiffschedulefrom.setText("Tiffin Scheduled From: " + dt.format(customerOrder1.getDate()));
	}
	...
	private void showCard2(CustomerOrder customerOrder12) {
		...
		tiffcard2menu.setText("Todays " + customerOrder12.getMealType() + " Menu of " + customerOrder12.getMeal().getVendor().getName() + " :");
		if (customerOrder12.getStatus() == null || MealStatus.PREPARE.equals(customerOrder12.getStatus()))
			tiffcard2status.setText("Your " + customerOrder12.getMealType() + " is yet to be cooked. You can cancel your order.");
		else
			tiffcard2status.setText("Your " + customerOrder12.getMealType() + " is in " + customerOrder12.getMeal().toString() + " stage.");
		...
		tiffcard2schedulefrom.setText("Tiffin Scheduled From: " + dt.format(customerOrder12.getDate()));
	...
	private void ShowMenu(CustomerOrder customerOrder2) {
		new GetMealMenuAsyncTask(getActivity(), null, "Scheduled", customerOrder2).execute();
	}

	private void orderValidation() {
		if (customer.getScheduledOrder() == null || customer.getScheduledOrder().size() == 0) {
			return;
		}
		...
		for (CustomerOrder cOrder : customer.getScheduledOrder()) {
			if (cOrder.getStatus() == null) {
				...
				Toast.makeText(getActivity(), "Your Tiffin had been delivered. Please rate us!!", Toast.LENGTH_SHORT).show();
			}

			else if (OrderStatus.PAYABLE.equals(cOrder.getStatus())) {
				Toast.makeText(getActivity(), "You won't recieve todays tiffin due to insufficient balance.", Toast.LENGTH_SHORT).show();
			}

			else if (OrderStatus.NA.equals(cOrder.getStatus())) {
				Toast.makeText(getActivity(), "Your Order is not available for today.You orderd for " + cOrder.getDate().toString(), Toast.LENGTH_SHORT).show();
			}
